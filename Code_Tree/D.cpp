...
class BiTree{
	private:
		...
		void PreOrder(BiTreeNode* t,int turn);
...
//私有函数的接口的实现
void BiTree::PreOrder(BiTreeNode* t,int turn){//turn==0为左，turn==1为右 
	if (t){
//		cout<<t->data;//输出数据，表示已经访问过了
		if (t->LeftChild==NULL&&t->RightChild==NULL&&turn==0)
			counter++;
		PreOrder(t->LeftChild,0);//先序遍历左孩子
		PreOrder(t->RightChild,1);//先序遍历右孩子 
	}
}
